...
import {
  ButtonWrapper,
  DownloadAutogeneratedTemplateBox,
...
> = ({ agencyId, systems, downloadTemplate, isDownloading }) => {
  const [templateKeyToLoadingStatus, setTemplateKeyToLoadingStatus] =
    React.useState<{
      [key: string]: boolean;
    }>({});
  /**
   * Generic templates contain sheet tabs for all metrics
  ...
  const fetchTemplate = async (
    ...
    const templateKey: string =
      system + (isSinglePageTemplate === true ? "_SINGLE" : "_MULTI");
    setTemplateKeyToLoadingStatus((prev) => ({
      ...prev,
      [templateKey]: true,
    }));

    await downloadTemplate(system, isSinglePageTemplate, isGenericTemplate);
    setTemplateKeyToLoadingStatus((prev) => ({
      ...prev,
      [templateKey]: false,
    }));
  ...
  return (
    <>
      ...
      <p>
        ...
        spreadsheet or CSV file in a particular format, so we can build
      ...
      <CustomOrGenericTemplateRadioButtons>
        <InputLabelWrapper>
          ...
          />
          <label htmlFor="custom-templates">Custom</label>
        </InputLabelWrapper>
        <InputLabelWrapper>
          <input
          ...
          />
          <label htmlFor="generic-templates">Default</label>
        </InputLabelWrapper>
      </CustomOrGenericTemplateRadioButtons>
          ...
          return (
            <>
              {systems.map((system) => {
                ...
                const systemNamePage = `${systemName} ${
      ...
      <p>
        ...
        {SYSTEM_LOWERCASE}. If you download the templates from this page, they
        will automatically include tabs for metrics and breakdowns that you have
        enabled. If you choose to upload a set of CSV files, you will need to
        upload one file for each metric. See the{" "}
        <a
      ...
      <p>
        ...
        given a standard name. Refer to your template file for the valid sheet
        names. If you are uploading a set of CSV files, their filenames names
        should also exactly match these sheet names (e.g. admissions.csv).
      </p>
      <p>
      ...
      <p>
        ...
        your template file for the name of the new sheet, the name of the new
        column for category names, and the valid values for this column.
      </p>
      <p>
        Please only provide valid category names (as seen in your template file)
        in the new column. If your agency categorizes the metric differently,
        group any unmatched data into the <i>Other</i> category.
      </p>
...
> = ({ system }) => {
  ...
  return (
    <>
      ...
      {system === "SUPERVISION" ? (
        <p>
          Note: If you are able to {REPORT_VERB_LOWERCASE} separately for Parole
          and Probation, then you should fill out rows for both{" "}
